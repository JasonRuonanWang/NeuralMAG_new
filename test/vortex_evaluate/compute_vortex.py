...
from util.vortex_utils import get_winding, get_randspin_2D

from libs.misc import Culist, create_model, create_random_mask
import libs.MAG2305 as MAG2305
from libs.UNet_base import UNet as UNet_new
from libs.UNet_ori import UNet as UNet_ori
...
def load_unet_model(args):
    ...
    if args.model == "new":
        ...
        UNet = UNet_ori
        krn = 16
    model = UNet(kc=krn, inc=args.layers*3, ouc=args.layers*3).eval().to(device)
...
def initialize_models(args):
    ...
    if args.experiment == 'square':
        ...
        ex_type = 'sp'
    path0 = "./{}/size{}_{}/".format(args.model, args.w, ex_type)
    ...
    return film1, film2, test_model, path0, ex_type
...
if __name__ == '__main__':
    ...
    parser.add_argument('--model',      type=str,    default="new",     help='model type (new/ori) (default: new)')
    ...
    film1, film2, test_model, path0, ex_type = initialize_models(args)

    # Random seed list
    seeds_list = np.load(f"./seed/size{args.w}_{ex_type}/deg{args.deg}/stable_seed.npy")
    ...
    for rand_seed in tqdm(seeds_list):
        ...
        spin0 = get_randspin_2D(size=(args.w, args.w, args.layers), split=args.split, 
                                  add_perturbation=False, seed=rand_seed)
        film1.SpinInit(spin0 * mask)
        film2.SpinInit(spin0 * mask)
