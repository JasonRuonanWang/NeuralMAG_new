...
def train_loss_compute(x, y, model, Loss1, Loss2, epoch, is_last_batch, size, ex_path, mode):
    mask = create_mask(x)
    pred_y = model(x)
    loss1 = mse( ISLA(pred_y), y ) * mask #enlarge-scale predict Hd to label Hd 
    loss2 = mse( pred_y, SLA(y) ) * mask  #shrink-scale label Hd to predict Hd
    loss = ( (loss1 + 1000*loss2) ).mean()
    Loss1.update( loss1.mean().item(),  x.size(0) )
    Loss2.update( loss2.mean().item(),  x.size(0) )
    if epoch > 0 and epoch % 10 == 0 and is_last_batch:
        visualize(mode, epoch, ex_path, x.detach(), y.detach(), ISLA(pred_y).detach(), size)
    del x, pred_y
    torch.cuda.empty_cache()
    return loss, loss1.detach().cpu().numpy()

def eval_loss_compute(x, y, model, Loss, epoch, is_last_batch, size, ex_path, mode):
    mask = create_mask(x)
    pred_y = model(x)
    loss = mse( ISLA(pred_y), y ) * mask #enlarge-scale predict Hd to label Hd
    mean_loss = loss.mean() 
    Loss.update( mean_loss.item(),  x.size(0) )
    if epoch > 0 and epoch % 10 == 0 and is_last_batch:
        visualize(mode, epoch, ex_path, x.detach(), y.detach(), ISLA(pred_y).detach(), size)
    del x, pred_y, loss
    torch.cuda.empty_cache()    
